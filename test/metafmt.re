...
module Metafmt = {
  ...
  let () =
    indexed_apply(
      Format.printf("Indexed apply: [%s %s %s %s %d]\n"),
      Meta.[Z, Z, Z, Z, S(Z)],
      L.["Hi", 1],
    );
};
...
let () = {
  ...
  xprintf("%d/%s %a", 2, "times", Format.pp_print_string, "to Ω");
  Format.printf("@.");
};

module Dyn = {
  let spec: format6(_) = ("%d %s %a %s %*.*f": format6(_));
  let x = "@[<v 2>@,Behold @{<greek>α@}:%3$a@;@[A text with a variable %1$i that appears %1$#x %2$s@]@;Printing variants can be precised by location,@;compare %4$s and %4$S.@;However, dynamic padding and precision are position-bound:@;%5$*.*g\n     @]";

  let parse = (spec, x) =>
    Parser.metafmt(Lexer.main, Lexing.from_string(x), spec);

  let y = parse(spec, x);

  let spec': format6(_) = ("%d %f %s": format6(_));
